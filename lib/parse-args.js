...
const buildYargs = async (args = [], withCommands = false) => {

  /**
   * configOptionConfigParser
   *
   * @param {*} path: absolute path to configuration file
   * @returns an empty object or a populated config object
   *
   * Note: This function returns an empty object when
   * dynamically importing an ESM config file.  In
   * this case a promise is assign and should be
   * resolved
   */

  let configPromise;
  const c8OptionParser = (path) => {
    let config = {}
    const configExt = extname(path).toLowerCase()
    if (configExt === '.mjs') {
      configPromise = async () => {
        return new Promise(async (promiseResolve) => {
          const loadedConfig = await loadConfigFile(path, configExt)
          config = applyExtends(loadedConfig, process.cwd(), true)
          promiseResolve(config)
        })
      }
    } else {
      loadedConfig = syncLoadConfigFile(path, configExt)
      config = applyExtends(loadedConfig, process.cwd(), true)
    }
    return config
  }
  ...
  .demandCommand(1)
  .check((argv) => {
    // This line doesn't do anything.  Going to ignore for now
    /* c8 ignore next 4 */
  ...
  const returnPromise = new Promise( async (promiseResolve) => {
    const parsedConfig = yargs.parse(args)
    if (configPromise) {
      fileConfig = await configPromise()
      config = Object.assign(parsedConfig, fileConfig)
      promiseResolve(config)
    } else {
      promiseResolve(parsedConfig)
    }
  })
...
const syncLoadConfigFile = (path, ext) => {
  let config = {}
  const jsExts = ['.js', '.cjs']
...
const loadConfigFile = async (path, ext) => {
  const c8Config = await import(path)
  const config = c8Config['default']
  return config
}
