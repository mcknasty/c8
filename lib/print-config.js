 ...
 *
 * Todo: Refactor regular expressions
 *
 */
function cleanUpArgumentObject (args) {
  ...
  const camelCaseKeys = keysToIterate.filter(x => {
    // Todo: Refactor reg-exp to their own files
 ...
 *  2.  Refactor reg-exp to their own file.
 *
 */
function printConfigBanner (cmdExecuted, configFilePath, tableWidth, tablePadding) {
  ...
  // Todo: Refactor reg-exp to their own file
  const reg = /\n{1} +/g
  const replacement = '\n' + graphicPaddingStr

  const banner = graphic.replace(reg, replacement) +
    summery.replace(reg, replacement)
 ...
 * @returns {String} - string representation of the variable.
 *
 * Todo: Refactor Regex into it's own file.
 *
 */
function stringifyObject (variable, space, closingChar) {
  ...
  // Todo: Refactor regular expression
