...
class Report {
  ...
  async getCoverageMapFromAllCoverageFiles () {
    ...
    const v8ProcessCov = (this.mergeAsync)
      ? await this._getMergedProcessCovAsync()
      : this._getMergedProcessCov()

    const resultCountPerPath = new Map()

    // Todo: Try block having trouble getting coverage.
    /* c18 ignore next 24 */
  ...
  async _getMergedProcessCovAsync () {
    const fileIndex = new Set() // Set<string>
    let mergedCov = null
    // Todo: Another try block that is having trouble getting coverage
